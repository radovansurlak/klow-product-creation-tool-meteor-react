...
Meteor.methods({
  async createMetafieldData({ productHandle, metafieldValueString }) {
    const authorizationString = base64(
      `${SHOPIFY_API_KEY}:${SHOPIFY_API_PASS}`,
    );

    const client = new GraphQLClient(
      `https://${SHOPIFY_STORE_NAME}.myshopify.com/admin/api/2019-10/graphql.json`,
      {
        credentials: 'include',
        mode: 'cors',
        headers: {
    ...
    try {
      ...
      throw new Meteor.Error('error requesting product ID query', null, {
        error,
        productHandle,
      });
    }
    ...
    if (productData.productByHandle && productData.productByHandle.id) {
      ...
      return new Meteor.Error('product ID undefined', null, {
        productData,
        productHandle,
      });
    }
    ...
    try {
      ...
      throw new Meteor.Error('error running metafield mutation query', null, {
        error,
        productHandle,
      });
    }
